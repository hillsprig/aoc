...
struct fmt::formatter<Point> {
    ...
    auto format(Point const &point, FormatContext &ctx) {
        return fmt::format_to(ctx.out(), "{}, {} s:{}", point.x, point.y, point.total_steps);
    }
};

template<>
struct fmt::formatter<Plot> {
    template<typename ParseContext>
    constexpr auto parse(ParseContext &ctx) {
        return ctx.begin();
    }
    ...
    template<typename FormatContext>
    auto format(Plot const &plot, FormatContext &ctx) {
        for (auto& p : plot) {
...
Movement stringToMovement(const std::string &str) {
    auto direction = static_cast<Direction>(str[0]);
...
void walk(Plot &plot, const Movement &mov) {
    ...
    int total_steps = p.total_steps;
    switch (mov.direction) {
        case Direction::Up:
            for (auto i = 1; i <= mov.distance; ++i) {
                plot.push_back({p.x, p.y + i, ++total_steps});
            }
            break;
        case Direction::Down:
            for (auto i = 1; i <= mov.distance; ++i) {
        ...
        case Direction::Right:
            for (auto i = 1; i <= mov.distance; ++i) {
                plot.push_back({p.x + i, p.y, ++total_steps});
            }
            break;
        case Direction::Left:
            for (auto i = 1; i <= mov.distance; ++i) {
...
int distanceToCentralPort(const Input& input) {
    ...
    for (auto& p : plot[0]) {
        auto match = std::find(plot[1].begin(), plot[1].end(), p);
        if (plot[1].end() != match) {
            int distance = std::abs(p.x) + std::abs(p.y);
                std::cout << fmt::format("{} : {}\n", distance, p.total_steps + match->total_steps);
        }
    }
